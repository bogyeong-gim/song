...
[PDM]
${proposalState.pdm.map(r => `[${r.level}] ${r.description} | 지표: ${r.indicator} | 가정: ${r.assumption}`).join('\n')}
...
[섹션 작성 현황]
${proposalState.sections.map(s => `- ${s.title}: ${s.content ? '작성됨' : '미작성'}`).join('\n')}
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'ODA Proposal 심사 전문가입니다. 요청된 JSON 형식으로만 응답합니다.',
        },
